...
- Agent 文本生成默认使用百炼固定快照 `qwen3.7-max-2026-05-20` 的 OpenAI 兼容 Chat Completions 接口；优先使用 2026-08-20 到期的免费额度。输出先由 JSON Mode 约束，再由本地 Pydantic 契约和证据编号范围二次校验。
...
- 新增可注入的内存 MES/QMS 假工具，用于后续 Agent 集成测试；当前没有连接或伪造真实生产系统。
...
- 结构化生成已完成一次真实百炼端到端冒烟，但尚无系统化引用忠实度、提示注入和生成质量评测；生成结论仍只基于知识库证据，不包含实时 MES/QMS 数据。
- MES/QMS 只读工具基础协议和假实现已完成，但尚未接入 Agent 状态机或真实业务系统；尚无工单写入、前端和 Agent 可观测性。
